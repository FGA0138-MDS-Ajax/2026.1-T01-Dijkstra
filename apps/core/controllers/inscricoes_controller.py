...
def gestao_inscricoes(request: HttpRequest, evento_id: uuid.UUID) -> HttpResponse:
    ...
    pendentes = Inscricao.objects.filter(evento=evento, status=Inscricao.Status.PENDENTE).count()
    aprovadas = Inscricao.objects.filter(evento=evento, status=Inscricao.Status.APROVADA).count()
    rejeitadas = Inscricao.objects.filter(evento=evento, status=Inscricao.Status.REJEITADA).count()
...
def aprovar_inscricao(request: HttpRequest, inscricao_id: uuid.UUID) -> HttpResponse:
    ...
    """
    _somente_organizador(request)
    inscricao = get_object_or_404(Inscricao.objects.select_related("evento"), pk=inscricao_id)
    ...
    if inscricao.status != Inscricao.Status.PENDENTE:
        ...
        messages.success(request, f"Inscrição de {inscricao.aluno.get_full_name() or inscricao.aluno.username} aprovada.")
...
def reprovar_inscricao(request: HttpRequest, inscricao_id: uuid.UUID) -> HttpResponse:
    ...
    if form.is_valid():
        ...
        inscricao.avaliador = request.user
        inscricao.data_avaliacao = timezone.now()
        inscricao.save(update_fields=["status", "avaliador", "data_avaliacao", "motivo_reprovacao"])
        messages.success(request, f"Inscrição de {inscricao.aluno.get_full_name() or inscricao.aluno.username} reprovada.")
    else:
...
def aprovar_todas_pendentes(request: HttpRequest, evento_id: uuid.UUID) -> HttpResponse:
    ...
    pendentes_qs = Inscricao.objects.filter(evento=evento, status=Inscricao.Status.PENDENTE)
...
def exportar_inscricoes_csv(request: HttpRequest, evento_id: uuid.UUID) -> HttpResponse:
    ...
    writer.writerow(["Nome", "E-mail", "Status", "Data da Solicitação", "Avaliador", "Data da Avaliação", "Motivo da Reprovação"])
    ...
    for inscricao in inscricoes:
        ...
        if inscricao.avaliador:
            avaliador_nome = inscricao.avaliador.get_full_name() or inscricao.avaliador.username

        writer.writerow([
            inscricao.aluno.get_full_name() or inscricao.aluno.username,
            inscricao.aluno.email,
            inscricao.get_status_display(),
            inscricao.data_solicitacao.strftime("%d/%m/%Y %H:%M") if inscricao.data_solicitacao else "",
            avaliador_nome,
            inscricao.data_avaliacao.strftime("%d/%m/%Y %H:%M") if inscricao.data_avaliacao else "",
